...
function LineCircuit(current_line, x, y, z, angle_x, angle_y, angle_z) {
    ...
    /*current_line.arrow.rotation.x = angle_x;
    current_line.arrow.rotation.y = angle_y;
    current_line.arrow.rotation.z = angle_z;*/
    ...
    /*current_line.arrow.position.x = Object.is(undefined, x) ? 0 : x;
    current_line.arrow.position.y = Object.is(undefined, y) ? 0 : y;
    current_line.arrow.position.z = Object.is(undefined, z) ? 0 : z;*/

    return current_line;
}

function SquareCircuit(wires, scene, length, half_flag, x, y, z) {
    if (half_flag) {
        wires.push(
            Object.freeze(LineCircuit(new CurrentLine(scene, length)))
        );
        wires.push(
            Object.freeze(LineCircuit(new CurrentLine(scene, length), -length / 2, length / 2, 0, 0, 0, 90 / 180 * Math.PI))
        );
    } else {
        wires.push(
            Object.freeze(LineCircuit(new CurrentLine(scene, length), length / 2 + x, y, z))
        );
        wires.push(
            Object.freeze(LineCircuit(new CurrentLine(scene, length), x, length / 2 + y, z, 0, 0, 90 / 180 * Math.PI))
        );
        wires.push(
            Object.freeze(LineCircuit(new CurrentLine(scene, length), -length / 2 + x, y, z, 0, 0, 180 / 180 * Math.PI))
        );
        wires.push(
            Object.freeze(LineCircuit(new CurrentLine(scene, length), x, -length / 2 + y, z, 0, 0, 270 / 180 * Math.PI))
        );
    }
}
...
function Circuit(scene, circuit_name) {
    ...
    switch(circuit_name) {
        ...
        case 'angle':
            SquareCircuit(this.wires, scene, 200, true);
            break;

        case 'square':
            SquareCircuit(this.wires, scene, 200, false, 0, 0, 50);
            break;

        case 'cube':
            SquareCircuit(this.wires, scene, 200, false, 0, 0, 0);
            SquareCircuit(this.wires, scene, 200, false, 0, 0, 200);
            this.wires.push(
        ...
        case 'four_inductors':
            for (let i = 0; i < 8; ++i) {
                SquareCircuit(this.wires, scene, 100, false, 150, 0, (i - 4) * 20);
            }
            for (let i = 0; i < 8; ++i) {
                this.wires.push(
                    Object.freeze(LineCircuit(new CurrentLine(scene, 100), -200, 0, (i - 4) * 20, 0, 0, 0))
                );
                this.wires.push(
                    Object.freeze(LineCircuit(new CurrentLine(scene, 100), -150, 50, (i - 4) * 20, 0, 0, 270 / 180 * Math.PI))
                );
                this.wires.push(
                    Object.freeze(LineCircuit(new CurrentLine(scene, 100), -100, 0, (i - 4) * 20, 0, 0, Math.PI))
                );
                this.wires.push(
                    Object.freeze(LineCircuit(new CurrentLine(scene, 100), -150, -50, (i - 4) * 20, 0, 0, 90 / 180 * Math.PI))
                );
            }
            for (let i = 0; i < 6; ++i) {
                this.wires.push(
                    Object.freeze(LineCircuit(new CurrentLine(scene, 100), (i - 3) * 20, 0, -200, 0, 0, 0))
                );
                this.wires.push(
                    Object.freeze(LineCircuit(new CurrentLine(scene, 100), (i - 3) * 20, 50, -150, 90 / 180 * Math.PI, 0, 0))
                );
                this.wires.push(
                    Object.freeze(LineCircuit(new CurrentLine(scene, 100), (i - 3) * 20, 0, -100, Math.PI, 0, 0))
                );
                this.wires.push(
                    Object.freeze(LineCircuit(new CurrentLine(scene, 100), (i - 3) * 20, -50, -150, 270 / 180 * Math.PI, 0, 0))
                );
            }
            for (let i = 0; i < 6; ++i) {
                this.wires.push(
